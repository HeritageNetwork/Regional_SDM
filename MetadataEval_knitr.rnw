...
# figure out size of study area, expand if less than 100km across
bbox <- bb(studyAreaExtent)
studyAreaWidth <- bbox$xmax - bbox$xmin
studyAreaHeight <- bbox$ymax - bbox$ymin
if(studyAreaWidth < 100000){
  bbox <- bb(bbox, width = 100000, relative = FALSE)
  }
if(studyAreaHeight < 100000){
  bbox <- bb(bbox, height = 100000, relative = FALSE)
  }
