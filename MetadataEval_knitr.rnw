...
stateBoundPath <- "D:/RegionalSDM/other_spatial" #path of the state boundaries
extentmap <- readOGR(stateBoundPath, "StateBoundariesAlbersConicEqualArea", stringsAsFactors=FALSE) # name of state boundaries file

#testareapath <- "X:/RegionalSDM/zz_testArea/inputs/background"
#testarea <- readOGR(testareapath,"clpBnd_SDM")

MapScipanci <- raster("D:/RegionalSDM/outputs/outras/glypmuhl.tif")
myTheme=rasterTheme(region=brewer.pal('Blues', n=9))
levelplot(MapScipanci, margin=FALSE, at=(0:10)/10, par.settings=rasterTheme(region=brewer.pal('Blues',n=9)), scales=list(draw = FALSE),xlim=c( 1000000, 2300000),ylim=c(1500000, 3100000)) + layer(sp.polygons(extentmap))
