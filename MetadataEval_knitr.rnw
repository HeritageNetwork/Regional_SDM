...
Distribution modeling by the Eastern Heritage Program Coalition
...
cat("Polys = input polygons; EOs = known locations; Groups = ", group$JackknType)
@
...
<<importanceFig, fig.width=3.0, fig.height=5.25, include=FALSE, echo=FALSE>>=
...
<<densityFig, fig.width=2, fig.height=1.5, include=FALSE, echo=FALSE>>=
...
\includegraphics{figure/densityFig-1.pdf}    %place it
...
\pagebreak
\medskip
<<mapFig, fig.width=5, fig.height=3, include=FALSE, echo=FALSE>>=
...
extentmap <- readOGR(stateBoundPath, "StatesNE")
region <- fortify(extentmap)
#get the prediction
map <- raster("X:/RegionalSDM/zz_testArea/outputs/grids/sciranci.tif")
map.p <- rasterToPoints(map) # convert the raster to points. Is there a better way???

#Make the points a dataframe for ggplot
df <- data.frame(map.p)
#Make appropriate column headings
colnames(df) <- c("Longitude", "Latitude", "MAP") #MAP in this case refers to mean annual precip... got to change

p <- ggplot(data=df, aes(y=Latitude, x=Longitude)) + 
     geom_raster(data=df, aes(fill=MAP)) +
     geom_polygon(data = region, aes(x=long, y=lat, group=group), color="gray", size=0.05,fill=NA) +
     labs(title = "Predicted Probability of Presence",fill="") +
     #scale_fill_gradient("probability", limits=c(0,1)) + #able for the legend
     scale_fill_distiller(palette = "Reds", direction=1) +
     #guides(fill = guide_legend(reverse=TRUE)) +
     coord_fixed(expand=TRUE) + #possibly a better way of doing this, but this squared up the map
     theme_nothing(legend=TRUE) # gets rid of the background grid
...
Figure 5. A coarse representation of the model, predicted throughout the study area. State Boundaries are shown in black. 


Please cite this document and its associated SDM as: \\
Regional Distribution Modeling Project \Sexpr{format(Sys.Date(), "%Y")}. Species distribution model, model validation, and environmental variable importance 
for \textit{\Sexpr{as.character(ElementNames[[1]])}}.  
