...
if (exists("n.var")) {
  ...
  txt <- "Validation was not possible for this model (too few reach groups)." 
}
@
...
\medskip
<<mapFig, fig.width=7.5, fig.height=6.5, include=FALSE, echo=FALSE>>=
...
par(mar=c(2,2,2,2.5), #bottom, left, top, right
    cex=0.9)    #text size
plot(results_shape, col = NA, axes = TRUE)
...
# plot(results_shape, col=results_shape$colcode, lwd = 0.15*results_shape$strord, add=T)
for (i in 1:nclr) {
  sub <- results_shape[results_shape$colcode == plotclr[i],]
  plot(sub, col=sub$colcode, lwd = 0.12*sub$strord, add=T)
}
# plot(spTransform(studyAreaExtent, CRSobj = results_shape@proj4string), add=T, border="red", lwd=0.2)
plot(spTransform( referenceBoundaries[referenceBoundaries$STATE_NAME != "Virginia",]
...
Figure 4. A generalized view of the model predictions throughout the study area. State boundaries are shown in grey. The Virginia border is outlined in black. 
...
\medskip
This model was built using a methodology developed through collaboration among the Florida Natural Areas Inventory, New York Natural Heritage Program, Pennsylvania Natural Heritage Program, and Virginia Natural Heritage Program. It is one of a suite of aqautic species distribution models developed using the same methods, scripts, and environmental data sets. Our goal was to be consistent and transparent in our methodology, validation, and output. This work was supported by the Virginia Tobacco Region Revitalization Commission.
...
\begin{thebibliography}{99}\setlength{\itemsep}{-4pt}
  ...
  \bibitem{streamcat} Hill, Ryan A., Marc H. Weber, Scott G. Leibowitz, Anthony R. Olsen, and Darren J. Thornbrugh. 
    The Stream-Catchment (StreamCat) Dataset: A Database of Watershed Metrics for the Conterminous United States. 2016. Journal of the American Water     Resources Association (JAWRA) 52:120-128.
...
<<tableFour, results="asis", echo = FALSE>>=
  ...
  print(xtable(sdm.var.info, label = NULL, caption = "Descriptions for environmental variables included in model."),
             include.rownames=FALSE, scalebox = 0.55, floating = TRUE, floating.environment = "sidewaystable")
@
\end{center}
