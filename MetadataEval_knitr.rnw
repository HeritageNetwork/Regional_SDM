...
<<mapFig, fig.width=7, fig.height=8, include=FALSE, echo=FALSE, dev='png', dpi=600>>=
...
basetiles <- read_osm(studyAreaExtent, type = mtype, ext = 1.1)
# plot it
qtm(basetiles) + 
  tm_shape(results_shape) + # results_shape$prbblty
    tm_lines(col="prbblty", palette=clrs, labels=c("Low Habitat Suitability", rep(" ", nclr-2), "High Habitat Suitability")) + # title="modeled suitability",    
  tm_shape(area.wb.pred) +
    tm_fill(col="pred", palette=clrs) + 
