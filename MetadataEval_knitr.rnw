...
@
BG reaches = background reaches; PR reaches = presence point placed in reaches.
\smallskip
\begin{center}
<<tableOne, results="asis", echo = FALSE>>=
	summ.table <- data.frame(Name=c("HUC12s","BG reaches","PR reaches"), #note how many HUC12 (replace poly and EOs, keep EOs/observations?, # length( unique(df.in2$huc12))))
                         Number=c(length(unique(df.in2$huc12)),
...
and background reaches, with an mtry of \Sexpr{rf.full$mtry}, and the same
...
\pagebreak
\medskip
<<mapFig, fig.width=7, fig.height=6, include=FALSE, echo=FALSE>>=
...
plot(results_shape, lwd = 0.05, col = "red", axes = TRUE)
...
for (i in 1:nclr) {
  ...
  plot(sub, col=sub$colcode, lwd = 0.15*sub$strord, add=T)
}
plot(spTransform(studyAreaExtent, CRSobj = results_shape@proj4string), add=T, border="red", lwd=0.2)
plot(spTransform(referenceBoundaries, CRSobj = results_shape@proj4string), add=T, border="gray80", lwd=0.2)
#title(main="Creek Heelsplittler" ) # sub=" Equal-Width Class Intervals"
legend("topleft", legend=names(attr(colcode, "table")), 
    col=attr(colcode, "palette"), cex=0.8, bty="o", lwd = 4, bg = "grey95")
@

\includegraphics{figure/mapFig-1.pdf}    
Figure 5. A generalized view of the model predictions throughout the study area. State boundaries are shown in grey. The study area is outlined in red. 
