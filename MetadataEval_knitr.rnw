...
\end{center}
\smallskip
Table 2. Input statistics. Presence locations are known locations for this species. These locations may be based on point observations (buffered based on spatial accuracy of the point) and polygon representations. Nearby locations are grouped together. Environmental conditions are randomly sampled within each of these groups the number of times noted (subsamples) to generate the total number of presence inputs (by tree for rf; for entire model for all others). Background points are placed throughout model area excluding known species locations. Algorithms incorporate background points differently; the number used for each is noted. 
\begin{center}
