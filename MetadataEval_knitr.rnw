...
<<tableOne, results="asis", echo = FALSE>>=
	summ.table <- data.frame(Name=c("Presence reaches","Reach groups","Background reaches"), #note how many reach groups (replace poly and EOs, keep EOs/observations?, # length( unique(df.in2$huc12))))
                         Number=c(nrow(subset(df.full, pres == 1)),length(group$vals),nrow(subset(df.full, pres == 0))
                         ))
...
qtm(basetiles) + 
  tm_shape(results_shape, name="modeled suitability") + # stream linework #, simplify = 0.5
    tm_lines(col="prbblty", palette=clrs, labels=c("Low Habitat Suitability", rep(" ", nclr-2), "High Habitat Suitability"), title.col = "modeled suitability") + {
      if(file.exists("aquaPolys")) tm_shape(aquaPolys) + tm_fill(col="prbblty", palette=clrs, legend.show = FALSE) 
  } +
  tm_shape(referenceBoundaries) +
  tm_borders(col = "grey", lwd = 1) +
  tm_shape(studyAreaExtent) + 
  tm_borders(col="red", lwd=2, alpha=1) +
