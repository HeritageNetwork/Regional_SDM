...
stateBoundPath <- "G:/RegionalSDM/_firstFullModelOut/outras" #path of the state boundaries
extentmap <- readOGR(stateBoundPath, "StatesNE", stringsAsFactors=FALSE) # name of state boundaries file

#testareapath <- "G:/RegionalSDM/_firstFullModelOut/outras"
#testarea <- readOGR(testareapath,"clpBnd_SDM")

MapScipanci <- raster("G:/RegionalSDM/_firstFullModelOut/outras/glypmuhl.tif")
myTheme=rasterTheme(region=brewer.pal('Blues', n=9))
levelplot(MapScipanci, margin=FALSE, at=(0:10)/10, par.settings=rasterTheme(region=brewer.pal('Blues',n=9)), scales=list(draw = FALSE),xlim=c( 1000000, 2300000),ylim=c(1500000, 3100000)) + layer(sp.polygons(extentmap)) #+ layer(sp.polygons(testarea,lwd=0.1, col='red'))

@
\includegraphics{figure/mapFig-1.pdf}    
Figure 5. A coarse overview of the model predictions throughout the study area. State Boundaries are shown in black.\\ 
...
\hangindent Northeast Regional Distribution Modeling Project. \Sexpr{format(Sys.Date(), "%Y")}. Species distribution model, model validation, and environmental variable importance 
for \textit{\Sexpr{as.character(ElementNames[[1]])}}.  
Created on \Sexpr{format(Sys.Date(), "%Y %b %d")}.
