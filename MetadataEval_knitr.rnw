...
model using all background and presence points as input. Abbreviations used: calc = calcareous, CP = coastal plain, dist = distance, fresh = freshwater, precip = precipitation, temp = temperature, max = maximum, min = minimum. See Table 4 for variable descriptions.
...
# set up plot
# need to union studyAreaExtent...
plot(studyAreaExtent, axes = F, col = NA, border = NA)
plot(referenceBoundaries, add = T, border = "grey80")
...
plot(studyAreaExtent, border = "red", lwd = 2, add = T)
@
