...
Table 1. Input statistics. PR points = presence points. These are points placed in polygon-based location information or point-based observations. Groups = groupings of points based on polygon data or spatial grouping of observations. BG points = background points placed throughout model area excluding known species locations.
\smallskip
\begin{center}
...
######
# library(ggmap)
# studyAreaExtent_ll <- sf::st_transform(studyAreaExtent,"+proj=longlat +datum=WGS84")
# bb <- st_bbox(studyAreaExtent_ll)
# names(bb) <- c("left","bottom","right","top")
# basem <- get_map(location = bb, source = "stamen", maptype = "toner")
# 
# ggmap(basem, extent = "device") +
#   geom_sf(data = studyAreaExtent_ll, inherit.aes = FALSE, 
#           fill = NA, colour = "red", size = 2) + 
#   geom_raster(ras, inherit.aes = FALSE)

atRamp <- seq(0,1,0.2)
#names(atRamp) <- c("Low","","","","","High")
mapviewOptions(legend.pos = "bottomleft", leafletHeight = 650, leafletWidth = 650)

mv <- mapview(studyAreaExtent, alpha.regions = 0, color = "red",
        map.types = c("Stamen.TonerLite"),
        homebutton = FALSE, legend.opacity = 0) +
    mapview(ras, maxpixels = 300000, col.regions = clrs, at = atRamp,
        na.color = NA, legend = TRUE, layer.name = "Suitability",
        homebutton = FALSE, legend.opacity = 0)

## need phantomjs installed to work:
## webshot::install_phantomjs()

#knit_print(mv)
mapshot(mv, file = here("_data","species",model_species,"outputs","metadata","figure","mapFig-1.png"))
#,
#        vwidth = 460, vheight = 500)
#  mapview(referenceBoundaries, alpha.regions = 0, color = "grey")
...
\begin{itemize}
  ...
  \item{Validation metrics requiring a threshold use maxSSS\cite{LiuEtAl2015}}
