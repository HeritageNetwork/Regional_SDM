...
\end{center}
Figure 1. Relative importance of each environmental variable based on the full model using all sites as input. Importance values are extracted from each algorithm and plotted here, in descending order based on an average of all algorithms (grey line). Note each algorithm has its own variable removal rules (see Appendix 2); missing points indicate the variable was not used by that algorithm. See Appendix 1 for variable descriptions.
\end{minipage}% 
