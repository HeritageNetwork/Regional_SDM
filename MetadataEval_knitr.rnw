...
area under the ROC curve; see \cite{fielding2002, allouche2006, vaughan2005}.
\smallskip
...
\medskip
<<checkEvalExists, echo=FALSE>>=
if (exists("n.var")) {
txt <- "Validation runs used \\Sexpr{n.var} environmental 
...
} else { txt <- "Validation was not possible for this model (too few EOs)." }
@
...
<<importanceFig, fig.width=3.0, fig.height=6.72, include=FALSE, echo=FALSE>>=
...
model using all background and presence points as input. Abbreviations used: calc = calcareous, CP = coastal plain, dist = distance, fresh = freshwater, precip = precipitation, temp = temperature, max = maximum, min = minimum.
...
levelplot(ras, margin=FALSE, at=(0:10)/10, par.settings=rasterTheme(region=brewer.pal('Blues',n=9)), scales=list(draw = FALSE),xlim=c(as.vector(studyAreaExtent@bbox[1,1])-10000,as.vector(studyAreaExtent@bbox[1,2])+10000),ylim=c(as.vector(studyAreaExtent@bbox[2,1])-10000,as.vector(studyAreaExtent@bbox[2,2])+10000)) + layer(sp.polygons(referenceBoundaries,lwd=0.4,col='gray50'), data=list(referenceBoundaries=referenceBoundaries)) + layer(sp.polygons(studyAreaExtent,lwd=1, col='red'), data = list(studyAreaExtent=studyAreaExtent))
@
