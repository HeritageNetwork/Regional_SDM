...
<<pPlotFig, fig.width=6.5, fig.height=6.5, include=FALSE, echo=FALSE>>=
...
layout(matrix(c(19,2,4,6,20,19,1,3,5,20,19,8,10,12,20,19,7,9,11,20,19,14,16,18,20,19,13,15,17,20), 
		...
		widths = c(0.15,1,1,1,0.1),heights=c(1,4,1,4,1,4))
...
levelplot(ras, margin=FALSE, at=(0:10)/10, par.settings=rasterTheme(region=brewer.pal('Blues',n=9)), scales=list(draw = FALSE),xlim=c( 1000000, 2300000),ylim=c(1500000, 3100000)) + layer(sp.polygons(extentmap,lwd=0.05,col='gray')) + layer(sp.polygons(testarea,lwd=1, col='red'))
