...
# load data, QC ----
fileName <- fileList[[n]]
presPolys <- read.csv(fileName)

#__Probablydontneed___  shpName <- strsplit(fileName,"\\.")[[1]][[1]]
#__Probablydontneed___  sppCode <- shpName
#__Probablydontneed___  presPolys <- readOGR(fileName, layer = shpName) #Z-dimension discarded msg is OK
#__Probablydontneed___  check for proper column names. If no error from next code block, then good to go
...
#__Probablydontneed___  get projection info for later
#__Probablydontneed___  projInfo <- presPolys@proj4string

#__Probablydontneed___   explode multi-part polys ----
#__Probablydontneed___  shp_expl <- disaggregate(presPolys)

#add some columns (explode id and area)
#__Probablydontneed___  shp_expl@data <- cbind(shp_expl@data, 
#__Probablydontneed___  EXPL_ID = rownames(shp_expl@data), 
#__Probablydontneed___  	AREAM2 = sapply(slot(shp_expl, "polygons"), slot, "area"))
		
#__Probablydontneed___   projection info doesn't stick, apply from what we grabbed earlier
#__Probablydontneed___   shp_expl@proj4string <- projInfo
#write out the exploded polygon set
#__Probablydontneed___  nm.PyFile <- paste(sppCode, "_expl", sep = "")
#__Probablydontneed___  writeOGR(shp_expl, dsn = ".", layer = nm.PyFile, driver="ESRI Shapefile", overwrite_layer=TRUE)
