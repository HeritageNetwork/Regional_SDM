...
setMethod('predictRF', signature(object='Raster'), 
	function(object, model, filename="", fun=predict, ext=NULL, const=NULL, index=1, se.fit=FALSE, na.rm=TRUE, progress='', ...) {
		...
		lyrnames <- names(object)  ####changed layerNames(object) to names(object)
		
		haveFactor <- FALSE
		## customized for RandomForest using x,y interface
		#dataclasses <- try (attr(model$terms, "dataClasses")[-1], silent=TRUE)
		dataclasses <- try (attr(model$forest$ncat, "names"), silent=TRUE)
		if (class(dataclasses) != "try-error") {
			...
			f <- names( which(model$forest$ncat != 1) )
			if (length(f) > 0) { 
				haveFactor <- TRUE 
				factlevels <- list()
				for (i in 1:length(f)) {
		...
		napred <- rep(NA, ncol(predrast)*tr$nrows[1] )  ###Changed tr$size to tr$nrows
