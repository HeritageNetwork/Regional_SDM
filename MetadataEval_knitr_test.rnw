...
\smallskip
\noindent
This SDM incorporates the number of known and background locations indicated in Table 2., modeled using the algorithms in Table 1 in the R statistical environment. We validated the model by jackknifing (also called leave-one-out {\protect\NoHyper\cite{fielding1997, fielding2002, pearson2007}\protect\endNoHyper}) by \Sexpr{tolower(as.character(group$JackknType))} for a total of \Sexpr{length(group$vals)} groups. The statistics in Table 3 report the mean and variance of validation statistics for these jackknifing runs.\\
\smallskip
\small
...
\end{center}
\smallskip
Table 3. Validation statistics for jackknife trials. AUC = 
area under the ROC curve, Sens = sensitivity, Spec = specificity, TSS = 
True Skill Statistic {\protect\NoHyper\cite{allouche2006, vaughan2005,
...
<<importanceFig, fig.width=3.2, fig.height=7.0, include=FALSE, echo=FALSE>>=
