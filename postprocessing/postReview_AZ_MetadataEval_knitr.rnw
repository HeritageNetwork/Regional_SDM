...
\begin{minipage}[b]{4.75in}   %everything in this minipage will be adjacent, left of the thermometer
  ...
  Date: \Sexpr{format(Sys.Date(), "%d %b %Y")}; Code: \Sexpr{as.character(ElementNames$Code)} (EGT_ID: \Sexpr{as.character(ElementNames$EGT_ID)})
\end{minipage}  \hfill   
...
This HSM was modeled using the algorithms in Table 1 using R{\protect\NoHyper\cite{r}\protect\endNoHyper}, 
incorporating the number of known and background locations indicated in Table 2. We validated the model by jackknifing {\protect\NoHyper\cite{fielding1997, fielding2002, pearson2007}\protect\endNoHyper} by \Sexpr{tolower(as.character(group$JackknType))} and testing \Sexpr{length(group$vals)} groups. Table 3 reports the validation statistics for these jackknifing runs.\\
\smallskip
\small
\nopagebreak[4]
\begin{minipage}[t]{3in}
\smallskip
Expert reviewers indicated \Sexpr{ifelse(length(final_Alg_List)>1,"these algorithms","this algorithm")} performed the best: \Sexpr{paste(final_Alg_List,sep=" ", collapse = ", ")}. The reported TSS (above and Table 3b) is based on the reviewer-defined threshold for \Sexpr{ifelse(length(final_Alg_List)>1,"these models","this model")}. Some data are provided for all models run, but the focus of this metadata report will be on \Sexpr{ifelse(length(final_Alg_List)>1,"these models","this model")} chosen by expert review.

\smallskip
Table 1. These algorithms were created for review. If multiple passed review, those were ensembled. The specific implementations are listed in the package column.
\begin{center}
...
@
\end{center}
Table 2. Input statistics. Presence locations may be based on point observations buffered by their spatial accuracy and polygons with nearby locations grouped together. Environmental conditions are randomly subsampled within each of these groups as noted to generate the number of presence inputs (by tree for rf; for entire model for others). Background points are placed throughout model except for presence locations. 
\begin{center}
...
@
\end{center}
Table 3. Mean validation statistics for jackknife trials (+/- standard deviation). AUC = area under the ROC curve, Sens = sensitivity, Spec = specificity, TSS = True Skill Statistic {\protect\NoHyper\cite{allouche2006, vaughan2005, fielding2002}\protect\endNoHyper}.
\begin{center}
...
@
\end{center}
Table 3b. Validation statistics for expert-chosen \Sexpr{ifelse(length(final_Alg_List)>1,"algorithms and thresholds","algorithm and threshold")}. TSS = True Skill Statistic {\protect\NoHyper\cite{allouche2006}\protect\endNoHyper}.
\begin{center}
<<tableThreeBee, results="asis", echo = FALSE>>=
  print(xtable(expertTSS), 
        floating=FALSE, include.rownames=FALSE)
@
\end{center}
\end{minipage}%
...
legend("bottomright", final_Alg_List, 
       col = figSpecs[match(final_Alg_List,figSpecs$algos),"col"],
       lwd = figSpecs[match(final_Alg_List,figSpecs$algos),"lwd"],
       lty = figSpecs[match(final_Alg_List,figSpecs$algos),"lty"],
       text.col = "black", 
...
Table 2. Reviewer star ratings for all model outputs of the accepted model version. More than one row for an algorithm indicates more than one reviewer. Average star ratings of 3 or more triggered the use of that algorithm in the final output. 


<<tableAppendixTwoTableTwo, results="asis", echo = FALSE>>=
  print(xtable(reviewer_rankings),
                floating=FALSE, include.rownames=FALSE)
@
\end{minipage}
